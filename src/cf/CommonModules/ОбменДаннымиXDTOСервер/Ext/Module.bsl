...
Функция ОбъектXDTOИзДанныхXDTO(
		...
		СсылкиИзОбъекта = Неопределено) Экспорт
	...
	Для Каждого Свойство Из ТипXDTO.Свойства Цикл
		...
		Если ТипСвойства = "ОбщееСоставноеСвойство" Тогда
			...
			ЗначениеXDTO = ОбъектXDTOИзДанныхXDTO(КомпонентыОбмена, Источник, Свойство.Тип,, СсылкиИзОбъекта);
...
Функция СтруктураОбъектаXDTOВДанныеИБ(КомпонентыОбмена, ДанныеXDTO, ПравилоКонвертации, Действие = "КонвертироватьИЗаписать") Экспорт
	...
	Если Действие = "КонвертироватьИЗаписать" Тогда
		...
		Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена
			...
			И ВариантИдентификации = "СначалаПоУникальномуИдентификаторуПотомПоПолямПоиска" Тогда
		...
		КонецЕсли;
		Если ДанныеДляЗаписиВИБ.ПометкаУдаления Тогда
			ДанныеДляЗаписиВИБ.ПометкаУдаления = Ложь;
		КонецЕсли;
		
		МетаданныеОбъекта = ДанныеДляЗаписиВИБ.Метаданные();
		...
		Иначе
			...
			КомпонентыОбмена.ОбъектыДляОтложеннойЗаписи.Вставить(
				ДанныеДляЗаписиВИБ.Ссылка, ДанныеДляЗаписиВИБ.ДополнительныеСвойства);
...
//
Процедура УстановитьОбменДаннымиЗагрузка(Объект, Значение = Истина, Знач ОтправкаНазад = Ложь, УзелОбменаЗагрузкаДанных = Неопределено) Экспорт
...
Процедура ЗаписатьОбъектВИБ(КомпонентыОбмена, Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь, УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		УстановитьОбменДаннымиЗагрузка(Объект,, ОтправкаНазад, КомпонентыОбмена.УзелКорреспондента);
	Иначе
...
// Создает таблицу значений для хранения правил конвертации свойств объекта.
//
// Возвращаемое значение:
//  Таблица значений 
//
...
Функция ФорматОбмена(Знач УзелИнформационнойБазы, Знач ВерсияФормата) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(ЭлементыФормата, "/");
КонецФункции
			...
			Для Каждого ПКС Из ТаблицаПКС Цикл
				...
				Если ЗначениеЗаполнено(ПравилоКонвертации.ПоляПоиска) Тогда
					Для Каждого ЭлементПолейПоиска Из ПравилоКонвертации.ПоляПоиска Цикл
						ПоляПоискаМассивом = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ЭлементПолейПоиска, ",",, Истина);
...
Процедура КонвертацияСвойстваXDTOВЭлементСтруктуры(Источник, Свойство, Приемник, ИмяДляСвойстваСоставногоТипа = "")
	...
	ИначеЕсли ТипЗнч(ЗначениеXDTO) = Тип("ОбъектXDTO") Тогда
		...
		ИначеЕсли Найти(ЗначениеXDTO.Тип().Имя, "КлючевыеСвойства") > 0 Тогда
		...
		ИначеЕсли Найти(ЗначениеXDTO.Тип().Имя, "ОбщиеСвойства") > 0 Тогда
...
Функция ТаблицаОбъектаПоТипу(Знач Тип)
	...
	Для Каждого Колонка Из Тип.Свойства Цикл
		...
		Если Найти(Колонка.Тип.Имя, "ОбщиеСвойства") > 0 Тогда
...
Функция СсылкаОбъектаПоСвойствамОбъектаXDTO(ПравилоКонвертации, ПолученныеДанные)
	...
	Если ПравилоКонвертации.ПоляПоиска = Неопределено ИЛИ
		ТипЗнч(ПравилоКонвертации.ПоляПоиска) <> Тип("Массив") Тогда
		Возврат Результат;
	КонецЕсли;
	...
	Для Каждого ПопыткаПоиска Из ПравилоКонвертации.ПоляПоиска Цикл
		...
		Иначе
			...
			УсловиеОтбора = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(Отбор, " И ");
...
Процедура ЗаполнитьДанныеИБПоПолученнымДанным(ДанныеИБ, ПолученныеДанные, ПравилоКонвертации)
	...
	Если ПоляКопии.Количество() > 0 Тогда
		ПоляКопии = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(ПоляКопии, ",", Истина);
...
Процедура ЗаполнитьТабличнуюЧастьОбъектаНачальнымиДанными(
	...
	Если ТипЗнч(КлючевыеПоля) = Тип("Строка") Тогда
		...
		Иначе
			КлючевыеПоля = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(КлючевыеПоля, ",");
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтложенноеЗаполнениеОбъектов(КомпонентыОбмена)
	...
	Для Каждого СтрокаТаблицы Из ЗагруженныеОбъекты Цикл
		...
		Если ОбъектМодифицирован Тогда
			УстановитьОбменДаннымиЗагрузка(Объект, Истина, Ложь, КомпонентыОбмена.УзелКорреспондента);
			Объект.Записать();
		КонецЕсли;
...
Процедура ВыполнитьОтложеннуюЗаписьОбъектов(КомпонентыОбмена)
	...
	Для Каждого ОбъектСоответствия Из КомпонентыОбмена.ОбъектыДляОтложеннойЗаписи Цикл
		...
		// Устанавливаем узел-отправитель для предотвращения регистрации объекта на узле, для которого производим загрузку
		// проведение выполняем не в режиме загрузки.
		УстановитьОбменДаннымиЗагрузка(Объект, Ложь, Ложь, КомпонентыОбмена.УзелКорреспондента);
...
Функция РазложитьФорматОбмена(Знач ФорматОбмена)
	...
	Результат.БазовыйФормат = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(ЭлементыФормата, "/");
...
Процедура ПрименитьУдалениеОбъектов(КомпонентыОбмена, МассивОбъектовКУдалению, МассивЗагруженныхОбъектов)
	...
	Для Каждого ЭлементКУдалению Из МассивОбъектовКУдалению Цикл
		...
		Иначе
			...
			ТипПолученныхДанныхСтрокой = ИмяТипаДанныхПоОбъектуМетаданных(Объект.Метаданные(), Ложь, Ложь);
...
//
Процедура ПриОбработкеПОД(КомпонентыОбмена, ПравилоОбработки, Знач ОбъектОбработки, ИспользованиеПКО) Экспорт
...
//
Функция ВыборкаДанных(КомпонентыОбмена, ПравилоОбработки) Экспорт
...
//  СтекВыгрузки     - Массив, содержит ссылки на выгружаемые объекты с учетом вложенности.
//
Процедура ПриОтправкеДанных(ДанныеИБ, ДанныеXDTO, Знач ИмяОбработчика, КомпонентыОбмена, СтекВыгрузки) Экспорт
	МенеджерОбмена = КомпонентыОбмена.МенеджерОбмена;
	СтруктураПараметров = Новый Структура();
