...
Функция ЗаменитьСсылки(Знач ПарыЗамен, Знач Параметры = Неопределено) Экспорт
	...
	Конфигурация.Вставить("МетаПоследовательности", Метаданные.Последовательности);
...
Процедура РазметитьМестаИспользования(Знач Конфигурация, Знач Ссылка, Знач ЦелеваяСсылка, Знач ТаблицаПоиска, РезультатРазметки = Неопределено)
	...
	МетаПоследовательности = Конфигурация.МетаПоследовательности;
	...
	Для Каждого МестоИспользования Из РезультатРазметки.МестаИспользования Цикл
		...
		ИначеЕсли МетаПоследовательности.Содержит(МетаданныеМеста) Тогда
...
Процедура ЗаменитьСсылкуДлиннойТранзакцией(РезультатЗамены, Знач Ссылка, Знач ПараметрыЗамены, Знач ТаблицаПоиска)
	...
	МестаИспользованияПоследовательности = ТаблицаПоиска.НайтиСтроки(Новый Структура("Ссылка, КлючЗамены", Ссылка, "Последовательность"));
	ДобавитьОбъектыБлокировкиПоследовательности(Блокировка, МестаИспользованияПоследовательности);
	...
	Если СостояниеОперации = "" Тогда
		...
		Если КоличествоОшибок = РезультатЗамены.Количество() Тогда
			Для Каждого МестоИспользования Из МестаИспользованияПоследовательности Цикл
				ПроизвестиЗаменуВНаборе(РезультатЗамены, МестоИспользования, ПараметрыЗаписи, Ложь);
				Если КоличествоОшибок <> РезультатЗамены.Количество() Тогда
					Прервать;
				КонецЕсли;
...
Процедура ДобавитьОбъектыБлокировкиПоследовательности(Блокировка, Знач СтрокиИспользования)
