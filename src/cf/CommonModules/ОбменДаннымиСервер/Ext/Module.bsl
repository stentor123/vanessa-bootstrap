...
//
Процедура ПриОшибкеПроверкиИдентификаторовОбъектовМетаданныхВПодчиненномУзлеРИБ(ПриЗагрузке, ВызыватьИсключение = Ложь) Экспорт
	
	Справочники.ИдентификаторыОбъектовМетаданных.ПроверкаИспользования();
	
	Если ПриЗагрузке Тогда
		ВключитьПовторениеЗагрузкиСообщенияОбменаДаннымиПередЗапуском();
		...
		Если ВызыватьИсключение Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Из главного узла не загружены изменения справочника ""Идентификаторы объектов метаданных"":
				           ...
				           |зарегистрируйте данные для восстановления подчиненного узла РИБ и
				...
				НСтр("ru = 'Чтобы зарегистрировать данные для восстановления подчиненного узла РИБ, в главном узле
				           |в разделе ""Администрирование"" откройте ""Настройки синхронизации данных"", перейдите
				           |по ссылке ""Автономная работа"". По команде ""Состав отправляемых данных"" откройте
				           |форму ""Регистрация изменений"", в которой  выполните команду
				           |""Зарегистрировать данные для восстановления подчиненного узла РИБ"" (меню Еще).'"),
				НСтр("ru = 'Чтобы зарегистрировать данные для восстановления подчиненного узла РИБ, в главном узле
				           |в разделе ""Администрирование"" откройте ""Настройки синхронизации данных"", перейдите
				           |по ссылке ""Синхронизация данных"". По команде ""Состав отправляемых данных"" откройте
				           |форму ""Регистрация изменений"", в которой  выполните команду
				           |""Зарегистрировать данные для восстановления подчиненного узла РИБ"" (меню Еще).'")));
	...
	ИначеЕсли ВызыватьИсключение Тогда // Настройка в подчиненном узле ИБ при первом запуске.
			ТекстОшибки =
...
//  ВыполнятьЗагрузку      - Булево (необязательный) - флаг необходимости выполнять загрузку данных. Значение по
//                           умолчанию - Истина.
//  ВыполнятьВыгрузку      - Булево (необязательный) - флаг необходимости выполнять выгрузку данных. Значение по
//                           умолчанию - Истина.
...
//								Значение по умолчанию - значение из
//								РС.НастройкиТранспортаОбмена.Ресурс.ВидТранспортаСообщенийОбменаПоУмолчанию; если в РС значение не задано, то
//								значение по умолчанию - Перечисления.ВидыТранспортаСообщенийОбмена.FILE.
// 
...
Процедура ВыполнитьОбменДаннымиДляУзлаИнформационнойБазыЧерезФайлИлиСтроку(УзелИнформационнойБазы = Неопределено,
	...
	Если СтруктураНастроекОбмена.ПроизводитьЗагрузкуДанных Тогда
		...
		Если ПолноеИмяФайлаСообщенияОбмена = ""
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") = Истина Тогда
		...
		Если УзелИнформационнойБазы <> Неопределено Тогда
			...
			Попытка
				...
				// только загрузка
				ВыполнитьОбменДаннымиДляУзлаИнформационнойБазы(Отказ, УзелИнформационнойБазы, Истина, Ложь, ВидТранспорта);
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") = Истина Тогда
			...
			Если УзелИнформационнойБазы <> Неопределено Тогда
				...
				Если ВыполнитьВыгрузку Тогда
					...
					// только выгрузка
					ВыполнитьОбменДаннымиДляУзлаИнформационнойБазы(Ложь, УзелИнформационнойБазы, Ложь, Истина, ВидТранспорта);
...
Процедура ПриЗаполненииСпискаТекущихДелПредупрежденияСинхронизации(ТекущиеДела)
	...
	Если Не ПравоДоступа("Просмотр", Метаданные.РегистрыСведений.РезультатыОбменаДанными)
		Или МодульТекущиеДелаСлужебный.ДелоОтключено("ПредупрежденияПриСинхронизации") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПриЗаполненииСпискаТекущихДелНеобходимоОбновление(ТекущиеДела)
	...
	Если Не ПравоДоступа("Администрирование", Метаданные)
		Или МодульТекущиеДелаСлужебный.ДелоОтключено("НеобходимоОбновлениеОбменДанными") Тогда
		Возврат;
	КонецЕсли;
	МодульТекущиеДелаСервер = ОбщегоНазначения.ОбщийМодуль("ТекущиеДелаСервер");
...
Процедура ПриЗаполненииСпискаТекущихДелПроверитьСовместимостьСТекущейВерсией(ТекущиеДела)
	...
	Если Не ПравоДоступа("Редактирование", Метаданные.РегистрыСведений.ПравилаДляОбменаДанными)
		Или МодульТекущиеДелаСлужебный.ДелоОтключено("ПравилаОбмена") Тогда
		Возврат;
	КонецЕсли;
...
Процедура УдалитьФайлИзХранилища(ИдентификаторФайла) Экспорт
	
	УдалитьФайлы(ВременныйКаталогВыгрузки(ИдентификаторФайла));
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		МодульОбменДаннымиВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиВМоделиСервиса");
		МодульОбменДаннымиВМоделиСервиса.ПриУдаленииФайлаИзХранилища(СтруктураЗаписи);
	Иначе
		
		ПриУдаленииФайлаИзХранилища(СтруктураЗаписи);
...
//
Процедура ПриУдаленииФайлаИзХранилища(Знач СтруктураЗаписи)
