...
Функция КомпонентыОбмена(НаправлениеОбмена, ВерсияФорматаОбменаПриЗагрузке = "") 
	...
	КомпонентыОбмена.КлючСообщенияЖурналаРегистрации = НСтр("ru = 'Перенос данных через буфер обмена';
															|en = 'Data transfer via clipboard'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КомпонентыОбмена.ВерсияФорматаОбмена = ТекВерсияФормата;
	//ГЗ Борлас Кожевников_СС 21.07.2021 PMK10579-854
	////ГЗ Гигабайт Селюкин А.Ю. 15.06.2021 { Нам надо сохранить возможность использования версий типовой схемы, но добавить к ним свои
	////КомпонентыОбмена.XMLСхема = "http://v8.1c.ru/edi/edi_stnd/EnterpriseData/" + ТекВерсияФормата;
	////} ГЗ Гигабайт Селюкин А.Ю. 15.06.2021
	////ГЗ Гигабайт Селюкин А.Ю. 15.06.2021 {
	//// Определение пространства имен с учетом версий EnterpriseData УФОД (Гознак)
	//КомпонентыОбмена.XMLСхема = ГЗ_ОбменДаннымиXDTOСерверПовтИсп.ПространствоИменПоВерсииУниверсальногоФормата(ТекВерсияФормата);
	//Если КомпонентыОбмена.XMLСхема = Неопределено Тогда
	...
	////} ГЗ Гигабайт Селюкин А.Ю. 15.06.2021
	////КомпонентыОбмена.XMLСхема = "http://v8.1c.ru/edi/edi_stnd/EnterpriseData/" + ТекВерсияФормата;			
	Если ТекВерсияФормата = "1.9" Тогда
		КомпонентыОбмена.XMLСхема = "http://v8.gz.ru/EnterpriseData/1.9";		
	Иначе
		КомпонентыОбмена.XMLСхема = "http://v8.1c.ru/edi/edi_stnd/EnterpriseData/" + ТекВерсияФормата;	
	КонецЕсли;
	//ГЗ Борлас Кожевников_СС 	
	...
	Если МенеджерОбменаВнутренний Тогда
		...
		ВерсииФорматаОбмена.Вставить("1.9", МенеджерОбменаЧерезУниверсальныйФормат);
...
Функция ЗагрузитьДанныеИзXML(ЧтениеXML)
	...
	РезультатЗагрузки.ЗагруженныеОбъекты = КомпонентыОбмена.ЗагруженныеОбъекты.ВыгрузитьКолонку("Объект");
