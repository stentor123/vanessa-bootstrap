...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.НаименованиеВторойБазы = НаименованиеВторойБазы(ЭтотУзел);
...
Процедура ПослеПроверкиРасширенияРаботыСФайлами(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ПослеВыбораФайла", ЭтаФорма, ДополнительныеПараметры);
...
Процедура ПослеВыбораФайла(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныеФайлы.Количество() = 1 Тогда
		Объект.ИмяФайлаНастроекОбменаДанными = ВыбранныеФайлы[0];
...
&НаСервере
Функция НаименованиеВторойБазы(ЭтотУзел)
	...
	Запрос.УстановитьПараметр("ЭтотУзел",     ЭтотУзел.Ссылка);
...
&НаСервереБезКонтекста
Процедура ИнициализацияОбменаДаннымиНаКлиенте(Отказ, УзелИнформационнойБазы, ВидТранспортаСообщенийОбмена)
	
	// Выполняем выгрузку данных
	ОбменДаннымиСервер.ВыполнитьОбменДаннымиДляУзлаИнформационнойБазы(Отказ,
		УзелИнформационнойБазы, Ложь, Истина, ВидТранспортаСообщенийОбмена);
...
Процедура ЗавершениеПервогоЭтапаНастройкиОбменаЧерезОбычныеКаналыСвязи(Отказ)
	...
	Иначе
		...
		Если ВыполнитьОбменДаннымиСейчас Тогда
			...
			ИнициализацияОбменаДаннымиНаКлиенте(Отказ, Объект.УзелИнформационнойБазы, Объект.ВидТранспортаСообщенийОбмена);
...
Функция Подключаемый_СтраницаМастераНачало_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.ИмяФайлаНастроекОбменаДаннымиДляЗагрузки) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.ИмяФайлаНастроекОбменаДаннымиДляЗагрузки");
		Возврат Неопределено;
	КонецЕсли;
	...
	ИменаФайлов.Добавить(Объект.ИмяФайлаНастроекОбменаДаннымиДляЗагрузки);
...
Процедура ЗавершениеВыбораФайлаНастроекОбменаДаннымиДляЗагрузки(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	Объект.ИмяФайлаНастроекОбменаДаннымиДляЗагрузки = РезультатПомещенияФайлов.Имя;
